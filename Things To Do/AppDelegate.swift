...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        navigationBarAppearace.barTintColor = uicolorFromHex(rgbValue: 0x13acd5)
...
//        })
        ...
        if let sessionToken = UserDefaults.standard.object(forKey: "sessionToken") as? String {
            if !sessionToken.isEmpty {
                ...
                let initialViewController = storyboard.instantiateViewController(withIdentifier: "taskListNav")
                self.window?.rootViewController = initialViewController
                self.window?.makeKeyAndVisible()
            }
        }
        
        //If User not intrasted in login or signup
        if let skip = UserDefaults.standard.object(forKey: "skip") as? String {
            if !skip.isEmpty {
                self.window = UIWindow(frame: UIScreen.main.bounds)
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let initialViewController = storyboard.instantiateViewController(withIdentifier: "taskListNav")
