...
class EditTaskController: UIViewController {
    ...
    @IBAction func updateTask(_ sender: Any) {
        if(taskTitle.text?.isEmpty)! {
            ...
            taskDescriptionError.isHidden = true
            errorMsg.isHidden = true
            let updateTaskLoading = self.displayLoading();
            //Add Task Request
            let postString = "taskTitle=\(taskTitle.text!)&taskDescription=\(taskDescription.text!)&taskId=\(taskIdValue)"
            let updateTaskResp = doHttpRequest(url: "updateTask.php", method: "POST", params: postString);
            if !((updateTaskResp?.isEmpty)!) {
                let status = updateTaskResp?["status"] as! String
                if(status == "false") {
                    errorMsg.isHidden = false
                    errorMsg.text = updateTaskResp?["error"] as? String
                } else {
                    updateTaskLoading.dismiss(animated: false, completion: {
                ...
                errorMsg.isHidden = false
                errorMsg.text = "Error while updating task."
            }
            updateTaskLoading.dismiss(animated: false)
        }
    }
